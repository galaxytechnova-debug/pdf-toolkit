...
export default function WordToPdfPage() {
  ...
  async function onFiles(files) {
    setError("");
    if (!files || files.length !== 1) { setError("Select a single .docx file."); return; }
    const f = files[0];
    setBusy(true);
    try {
      const blob = await postMultipart("word-to-pdf", { file: f });
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-10">
      <div className="text-center mb-10">
        <h1 className="text-3xl font-bold tracking-tight text-zinc-900 mb-2">Word to PDF</h1>
        <p className="text-zinc-500">Convert your DOCX documents to professional PDFs.</p>
      </div>
      ...
      <UploadArea
        ...
        description="Drag & drop a .docx file to convert"
        color="fuchsia"
      />
      {busy && <div className="mt-8 text-center animate-pulse text-fuchsia-600 font-medium">Converting Document...</div>}
